...
export function CreateServiceDialog({
  ...
  const create = useMutation({
    mutationFn: (input: { name: string; description: string; amountPence: number; validFrom: string }) =>
      createService(input),
    onSuccess: (service) => {
        ...
        <form className="space-y-4" onSubmit={submit}>
          ...
          <DialogFooter>
            ...
            <Button type="submit" variant="premium" disabled={create.isPending}>
              {create.isPending ? <Loader2 aria-hidden="true" className="h-4 w-4 animate-spin" /> : null}
              Create service
            </Button>
          </DialogFooter>
        </form>
