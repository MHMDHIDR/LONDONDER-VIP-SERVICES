-- Update create_receipt_and_items to use business name initials for the receipt prefix

CREATE OR REPLACE FUNCTION public.create_receipt_and_items(
  _issue_date DATE,
  _customer_name TEXT,
...
BEGIN
  ...
  -- Fallback if prefix generation fails for some reason
  ...
  FOR _item IN SELECT * FROM jsonb_array_elements(_items) LOOP
    ...
    INSERT INTO public.receipt_items (
      receipt_id, position, name, description, quantity, unit_price_pence, line_total_pence
    ) VALUES (
      _receipt_id,
      _pos,
      btrim(_item->>'name'),
      NULLIF(btrim(COALESCE(_item->>'description','')), ''),
      _qty,
      _unit,
      _line
    );
  ...
  UPDATE public.receipts SET total_amount_pence = _subtotal WHERE id = _receipt_id;
